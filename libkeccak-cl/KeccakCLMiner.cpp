...
struct KeccakCLChannel : public LogChannel
{
    static const char* name() { return EthOrange "keccakcl"; }
    static const int verbosity = 2;
    static const bool debug = false;
};
#define keccakcllog clog(KeccakCLChannel)
...
KeccakCLMiner::KeccakCLMiner(unsigned _index, CLSettings _settings, DeviceDescriptor& _device)
  : Miner("keccakcl-", _index), m_settings(_settings)
{
...
KeccakCLMiner::~KeccakCLMiner()
{
    DEV_BUILD_LOG_PROGRAMFLOW(keccakcllog, "keccakcl-" << m_index << " KeccakCLMiner::~KeccakCLMiner() begin");
    stopWorking();
    kick_miner();
    DEV_BUILD_LOG_PROGRAMFLOW(keccakcllog, "keccakcl-" << m_index << " KeccakCLMiner::~KeccakCLMiner() end");
}
            ...
            if (results.count)
            {
                ...
                for (uint32_t i = 0; i < results.count; i++)
                {
                    ...
                    if (nonce != m_lastNonce)
                    {
                        ...
                        keccakcllog << EthWhite << "Job: " << current.header.abridged() << " Sol: 0x"
...
void KeccakCLMiner::enumDevices(std::map<string, DeviceDescriptor>& _DevicesCollection)
{
...
bool KeccakCLMiner::initDevice()
{
    ...
    else
    {
        ...
        keccakcllog << "Unrecognized Platform";
        return false;
    }
    if (!m_settings.noExit && (m_hwmoninfo.deviceType != HwMonitorInfoType::AMD))
    {
        m_settings.noExit = true;
        keccakcllog << "no exit option enabled for non AMD opencl device";
    }
            ...
            m_deviceDescriptor.clPlatformVersionMinor == 1))
    {
        ...
        {
            keccakcllog << "OpenCL " << m_deviceDescriptor.clPlatformVersion
...
bool KeccakCLMiner::initEpoch_internal()
{
    try
    {
        ...
        keccakcllog << "Keccak OpenCL kernel";
        ...
        keccakcllog << "Loading kernels";
        m_searchKernel = cl::Kernel(program, "search_keccak");
        
        // create buffer for header
        keccakcllog << "Creating buffer for header.";
        m_header.clear();
        ...
        keccakcllog << "Creating mining buffer";
        m_searchBuffer.clear();
        m_searchBuffer.emplace_back(m_context[0], CL_MEM_WRITE_ONLY, sizeof(SearchResults));
    }
    catch (cl::Error const& err)
    {
        keccakcllog << ethCLErrorHelper("OpenCL init failed", err);
        pause(MinerPauseEnum::PauseDueToInitEpochError);
        return false;
    }
    return true;
