...
#endif
#if ETH_SHA3CL
#include <libsha3-cl/Sha3CLMiner.h>
#endif
#if ETH_ETHASHCUDA
...
#undef BACKTRACE_MAX_FRAMES
    ...
    bool validateArgs(int argc, char** argv)
    {
...
#if ETH_ETHASHCL || ETH_SHA3CL || ETH_ETHASHCUDA || ETH_ETHASH_CPU

        app.add_flag("--list-devices", m_shouldListDevices, "");

#endif

#if ETH_ETHASHCL || ETH_SHA3CL
        ...
        bool cl_sha3_miner = false;
        app.add_flag("--sha3", cl_sha3_miner, "");
        ...
        else if (cl_sha3_miner)
            m_minerType = MinerType::SHA3CL;
        else if (cuda_miner)
...
#endif
#if ETH_SHA3CL
        if (m_minerType == MinerType::SHA3CL || m_minerType == MinerType::Mixed)
            Sha3CLMiner::enumDevices(m_DevicesCollection);
#endif
#if ETH_ETHASHCUDA
...
#endif
#if ETH_SHA3CL
            if (m_minerType == MinerType::SHA3CL)
                cout << setw(5) << "SHA3CL   ";
#endif
            cout << resetiosflags(ios::left) << setw(13) << "Total Memory"
                 << " ";
#if ETH_ETHASHCL || ETH_SHA3CL
            if (m_minerType == MinerType::CL || m_minerType == MinerType::SHA3CL || m_minerType == MinerType::Mixed)
            {
...
#if ETH_SHA3CL
        if (m_CLSettings.devices.size() &&
            (m_minerType == MinerType::CL || m_minerType == MinerType::SHA3CL || m_minerType == MinerType::Mixed))
        {
            for (auto index : m_CLSettings.devices)
            {
                if (index < m_DevicesCollection.size())
                {
                    ...
                    if (it->second.subscriptionType != DeviceSubscriptionTypeEnum::None)
                        throw std::runtime_error(
                            "Can't OpenCL subscribe a CUDA subscribed device.");
                    it->second.subscriptionType = DeviceSubscriptionTypeEnum::Sha3OpenCL;
                }
            }
        }
#endif
...
#endif
#if ETH_SHA3CL
             << "    --sha3              Mine/Benchmark using Sha3 OpenCL only" << endl
#endif
#if ETH_ETHASHCUDA
...
#endif
#if ETH_SHA3CL
             << "                        'sha3cl'   Extended Sha3 OpenCL options" << endl
#endif
#if ETH_ETHASHCUDA
