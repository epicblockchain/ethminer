...
)
{
    ...
    //if (gid == 59652352 || gid == 0) {
        //printf("gid: %d; header[0]: %#v2llx; header[3]: %#v2llx; target[0]: %#v2llx; target[3]: %#v2llx", gid, g_header[0], g_header[3], g_target[0], g_target[3]);
    //}
...
#endif
        //printf("solution nonce %08x%08x: %08x %08x", start_nonce, gid, state[0].x, state[0].y);
