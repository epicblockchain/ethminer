...
struct Sha3CLChannel : public LogChannel
{
    static const char* name() { return EthOrange "sha3cl"; }
    static const int verbosity = 2;
    static const bool debug = false;
};
#define sha3cllog clog(Sha3CLChannel)
...
Sha3CLMiner::Sha3CLMiner(unsigned _index, CLSettings _settings, DeviceDescriptor& _device)
  : Miner("sha3cl-", _index), m_settings(_settings)
{
...
Sha3CLMiner::~Sha3CLMiner()
{
    DEV_BUILD_LOG_PROGRAMFLOW(sha3cllog, "sha3cl-" << m_index << " Sha3CLMiner::~Sha3CLMiner() begin");
    stopWorking();
    kick_miner();
    DEV_BUILD_LOG_PROGRAMFLOW(sha3cllog, "sha3cl-" << m_index << " Sha3CLMiner::~Sha3CLMiner() end");
}
            ...
            if (results.count)
            {
                ...
                for (uint32_t i = 0; i < results.count; i++)
                {
                    ...
                    if (nonce != m_lastNonce)
                    {
                        ...
                        sha3cllog << EthWhite << "Job: " << current.header.abridged() << " Sol: 0x"
...
void Sha3CLMiner::enumDevices(std::map<string, DeviceDescriptor>& _DevicesCollection)
{
...
bool Sha3CLMiner::initDevice()
{
    ...
    else
    {
        ...
        sha3cllog << "Unrecognized Platform";
        return false;
    }
    if (!m_settings.noExit && (m_hwmoninfo.deviceType != HwMonitorInfoType::AMD))
    {
        m_settings.noExit = true;
        sha3cllog << "no exit option enabled for non AMD opencl device";
    }
            ...
            m_deviceDescriptor.clPlatformVersionMinor == 1))
    {
        ...
        {
            sha3cllog << "OpenCL " << m_deviceDescriptor.clPlatformVersion
...
bool Sha3CLMiner::initEpoch_internal()
{
    try
    {
        ...
        sha3cllog << "Sha3 OpenCL kernel";
        code = string(sha3_cl, sha3_cl + sizeof(sha3_cl));
        ...
        sha3cllog << "Loading kernels";
        m_searchKernel = cl::Kernel(program, "search_sha3");
        
        // create buffer for header
        sha3cllog << "Creating buffer for header.";
        m_header.clear();
